...
public class TextTruncator {
  ...
  private final View viewToCheckIfOverlapping;
   ...
   * @param viewToCheckIfOverlapping - view with which to check if given text view with string
   ...
   */
  public TextTruncator(String description, int maxLines, TextView textView, View
      viewToCheckIfOverlapping) throws IllegalArgumentException {
    if (description == null || maxLines <= 0 || textView == null || viewToCheckIfOverlapping ==
        null) {
    ...
    this.viewToCheckIfOverlapping = viewToCheckIfOverlapping;
  }
  ...
  public String getTruncatedString() {
    ...
    if (!description.equals(Constants.EMPTY_STRING) && truncateAt >= 0 && truncateAt <
      ...
      truncatedString = description.substring(0,
          truncateAt + 1) + (shouldAddElipsize ? Constants.ELLIPSIZE_END : Constants.EMPTY_STRING);
    }
  ...
  private void findTruncateAtPosition() {
    ...
    truncateAt = truncateTextViewToLines(description, textView,
        viewToCheckIfOverlapping, maxLines);
   ...
   */
  private int truncateTextViewToLines(String textContent, TextView textView,
                                      View viewToCheckForOverlapping, int maxLines) {
    if (textContent == null || textView == null || viewToCheckForOverlapping == null || textView
    ...
    if (lineCount >= maxLines) {
      ...
      boolean truncate = lineCount > maxLines || AndroidUtils.overlapsWithLine
          (textView, viewToCheckForOverlapping, maxLines - 1);
      if (truncate) {
        int start = textLayout.getLineStart(maxLines - 1), end = textLayout.getLineEnd
            (maxLines - 1);
        if (end < start) {
          return -1;
        }
        ...
        int leftWidth = textView.getWidth() - viewToCheckForOverlapping.getWidth() -
            elipsizeWidth - textView.getPaddingLeft();
        if (leftWidth < 0) {
          return start - 1;
        }
