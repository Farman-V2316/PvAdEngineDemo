 ...
 *
 * open source: https://github.com/psiegman/epublib
 *
 */
...
public class IOUtil {
  ...
  public static String convertToStorageMode(String aData, @NonNull String clientId,
                                            @NonNull String uDID) {
    String dataToReturn = aData;
    try {
      IOUtilsExtraLocal iOUtilsExtraLocal = new IOUtilsExtraLocal();
      byte[] dataToReturnBytes = iOUtilsExtraLocal.encryptDataWithTransferKey(
          aData.getBytes(Charset.forName("UTF-8")), clientId, uDID);
      dataToReturn = Base64.encodeToString(dataToReturnBytes,
          Base64.DEFAULT | Base64.NO_PADDING | Base64.NO_WRAP | Base64.URL_SAFE | Base64.NO_CLOSE);
    } catch (Exception e) {
      // //// e.printStackTrace();
    }
    return dataToReturn;
  ...
  public static String convertFromStorageMode(String aData, @NonNull String clientId,
                                              @NonNull String uDID) {
    String dataToReturn = aData;
    try {
      IOUtilsExtra iOUtilsExtra = new IOUtilsExtra();
      byte[] dataToDecrypt = Base64.decode(aData,
          Base64.DEFAULT | Base64.NO_PADDING | Base64.NO_WRAP | Base64.URL_SAFE | Base64.NO_CLOSE);
      byte[] bytesToReturn = iOUtilsExtra.decryptDataWithTransferKey(dataToDecrypt, clientId, uDID);
      dataToReturn = new String(bytesToReturn, Charset.forName("UTF-8"));
    } catch (Exception e) {
      // //// e.printStackTrace();
    }
    return dataToReturn;
