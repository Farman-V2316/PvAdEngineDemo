...
final public class MraidInterface {
  ...
  // Features that mraid ads can take advantage of, if this instance supports
	// them
	...
	public enum STATES {
		LOADING, DEFAULT, EXPANDED, RESIZED, HIDDEN, INVALID
	}
	...
	// Where an ad can be placed; inline (banner) or interstitial (full screen,
	// transitions)
	...
	// Orientation properties that can be set before / during
	// expand/interstitial ad display
	...
	// ALLOW_ABSOLUTE_OFFSET :: this variable is used for bypassing the relative
	// resize properties wrt banner/adView
	public enum RESIZE_PROPERTIES {
		WIDTH, HEIGHT, CUSTOM_CLOSE_POSITION, OFFSET_X, OFFSET_Y, ALLOW_OFF_SCREEN, ALLOW_ABSOLUTE_OFFSET, SKIP_CLOSE_BUTTON
	...
	// Calendar properties as specified by W3C used when creating a calendar
	// event
	...
	public static final String MRAID_ERROR_NO_ACTION_HANDLED = "Action not handled";
	...
	synchronized public void setDeviceFeatures() {
		...
		for (MraidInterface.FEATURES feature : MraidInterface.FEATURES.values()) {
			...
			webView.injectJavaScript("mraid.setSupports(\"" + name + "\", "
					+ deviceFeatures.isSupported(feature) + ");");
		}
	}
	...
	// Tell ad that an SDK side error has occurred; message describes error,
	// action optionally indicates name of action causing error
	public void fireErrorEvent(String message, String action) {
		// String name = get_EVENTS_name(EVENTS.ERROR);
		adView.getLog().log(MASTAdLog.LOG_LEVEL_DEBUG,
				"MraidInterface.errorEvent", message + ":" + action);
		webView.injectJavaScript("mraid.fireErrorEvent(\"" + message + "\",\""
				+ action + "\");");
	}

	// Tell ad that environment is ready, and change state from loading to
	// default
	public void fireReadyEvent() {
		// latest spec has switched the order so that state change goes before
		// ready event...
	...
	public void fireSizeChangeEvent(int width, int height) {
		...
		webView.injectJavaScript("mraid.fireSizeChangeEvent(" + width + ", "
				+ height + ");");
	}
	...
	public void firePictureAddedEvent(Boolean success) {
		...
		webView.injectJavaScript("mraid.fireAddedEvent(\"" + name + "\","
				+ success.toString() + ");");
	}
	...
	public void setState(STATES toState) {
		...
		if (null != adView) {
			adView.richmediaEvent(
					MASTAdConstants.CUSTOM_METHOD_SET_STATE_CHANGE, ""
							+ toState);
		}
	}

  public void stopVideo(){
    webView.injectJavaScript("if(typeof stopVideo != 'undefined'){" +
        "stopVideo();}");
  }
	...
	synchronized public void setCurrentPosition(int x, int y, int width,
			int height) {
		...
		try {
			...
			position.put(
					MraidInterface
							.get_CURRENT_POSITION_name(MraidInterface.CURRENT_POSITION.X),
					"" + x);
			position.put(
					MraidInterface
							.get_CURRENT_POSITION_name(MraidInterface.CURRENT_POSITION.Y),
					"" + y);
			position.put(
					MraidInterface
							.get_CURRENT_POSITION_name(MraidInterface.CURRENT_POSITION.WIDTH),
					"" + width);
			position.put(
					MraidInterface
							.get_CURRENT_POSITION_name(MraidInterface.CURRENT_POSITION.HEIGHT),
					"" + height);

			webView.injectJavaScript("mraid.setCurrentPosition("
					+ position.toString() + ");");

			// Log.e("MRAID_FLOW",
			// "Cust setCurrentCoOrdinates setCurrentPosition :  position " +
			// position.toString());
		} catch (Exception ex) {
			adView.getLog().log(MASTAdLog.LOG_LEVEL_ERROR,
					"JavascriptInterface setCurrentPosition - exception",
					ex.getMessage());
			// Log.e("MRAID_FLOW",
			// "Cust setCurrentCoOrdinates setCurrentPosition Execption");
		}

	}

	synchronized public void setCurrentPlacementContext(
			String aCurrentPlacementContext) {
		if (null != webView) {
			webView.injectJavaScript("mraid.setCurrentPlacementContext(\""
					+ aCurrentPlacementContext + "\");");
		}
	}
	...
	private void setPropertiesFromList(String name, List<NameValuePair> list) {
		...
		while (i.hasNext()) {
			...
			if ((nvp != null) && (nvp.getName() != null)) {
				try {
					...
					adView.getLog()
							.log(MASTAdLog.LOG_LEVEL_ERROR,
									"JavascriptInterface setPropertiesFromList - exception",
									ex.getMessage());
					return;
				}
			}
		...
		webView.injectJavaScript(function + json.toString() + ");"); // the JSON
																		// string
																		// format
																		// "just works"
																		// for
	...
	public void expand() {
		...
		webView.injectJavaScript("mraid.expand();");
	}

	//
	// Utility methods to convert enum values to strings matchng expected names
	...
	public static String get_FORCE_ORIENTATION_PROPERTIES_name(
			FORCE_ORIENTATION_PROPERTIES e) {
		return e.toString().toLowerCase();
	}

	public static String get_ORIENTATION_PROPERTIES_name(
			ORIENTATION_PROPERTIES o) {
		switch (o) {
	...
	public static FORCE_ORIENTATION_PROPERTIES get_FORCE_ORIENTATION_PROPERTIES_by_name(
			String forceOrientationPropertiesName) {
		if (forceOrientationPropertiesName != null) {
			for (FORCE_ORIENTATION_PROPERTIES p : FORCE_ORIENTATION_PROPERTIES
					.values()) {
				if (get_FORCE_ORIENTATION_PROPERTIES_name(p).equals(
						forceOrientationPropertiesName))
					return p;
			}
		}
	...
	public static RESIZE_CUSTOM_CLOSE_POSITION get_RESIZE_CUSTOM_CLOSE_POSITION_by_name(
			String positionName) {
		if (positionName != null) {
			for (RESIZE_CUSTOM_CLOSE_POSITION p : RESIZE_CUSTOM_CLOSE_POSITION
					.values()) {
				if (get_RESIZE_CUSTOM_CLOSE_POSITION_name(p).equals(
						positionName))
					return p;
			}
		}

		return RESIZE_CUSTOM_CLOSE_POSITION.TOP_RIGHT; // default position, or
														// null for error?
	}

	public static String get_RESIZE_CUSTOM_CLOSE_POSITION_name(
			RESIZE_CUSTOM_CLOSE_POSITION r) {
	...
	public static String get_CALENDAR_EVENT_PARAMETERS_name(
			CALENDAR_EVENT_PARAMETERS c) {
		return c.toString().toLowerCase();
	}
}
