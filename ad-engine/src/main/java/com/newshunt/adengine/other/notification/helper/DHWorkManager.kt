 ...
 */
package com.newshunt.adengine.other.notification.helper
...
import com.newshunt.adengine.other.model.entity.WorkManagerQueueEntry
...
object DHWorkManager {
    ...
    fun beginWork(workRequest: OneTimeWorkRequest, shouldReplaceExistingWork: Boolean = false) {
        if(donNotSchedule.get()){
            addToQueue(
                WorkManagerQueueEntry(
                    workRequest = workRequest,
                    shouldReplaceExistingWork = shouldReplaceExistingWork
                )
            )
